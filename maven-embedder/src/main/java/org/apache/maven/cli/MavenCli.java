...
public class MavenCli {
    ...
    public static Logger logger = LoggerFactory.getILoggerFactory().getLogger("MavenCli");
    ...
    public int doMain(CliRequest cliRequest) {
        logger.info("3、doMain,cliRequest:{}", JSON.toJSONString(cliRequest));
        PlexusContainer localContainer = null;
        try {
            /**
             * 初始化 workingDirectory multiModuleProjectDirectory maven.home
             */
            initialize(cliRequest);

            /**
             * 处理命令，将.mvn/maven.config和用户传递的命令合并
             */
            cli(cliRequest);
            /**
             * 处理环境变量 1、系统环境变量赋值 2、用户命令参数赋值到用户环境变量
             */
            ...
            version(cliRequest);
            /**
             * 构建容器，包括 classloader、classRealm，加载扩展文件（.mvn/extensions.xml） modelProcessor
             */
            ...
            /**
             * 处理各种命令和参数，加载项目pom文件、localRepoProperty
             */
    ...
    void initialize(CliRequest cliRequest) throws ExitException {
        ...
        logger.info("workingDirectory赋值：{}", cliRequest.workingDirectory);
        ...
        logger.info("multiModuleProjectDirectory 赋值：{}", cliRequest.multiModuleProjectDirectory);
        ...
        logger.info("mavenHome:{}", mavenHome);
        if (mavenHome != null) {
            String absolutePath = new File(mavenHome).getAbsolutePath();
            logger.info("setProperty mavenHome:{}", absolutePath);
            System.setProperty("maven.home", absolutePath);
        }
    }
    ...
    void cli(CliRequest cliRequest) throws Exception {
        ...
        try {
            ...
            if (configFile.isFile()) {
                ...
                logger.info(MVN_MAVEN_CONFIG + "中的命令", args);
                // 检查是否有不符合的命令
        ...
        try {
            CommandLine requestCommand = cliManager.parse(cliRequest.args);
            if (mavenConfig == null) {
                cliRequest.commandLine = requestCommand;
            } else {
                cliRequest.commandLine = cliMerge(requestCommand, mavenConfig);
            }
            logger.info("requestCommand:{},mavenConfig:{},merger:{}", JSON.toJSONString(requestCommand),
                JSON.toJSONString(mavenConfig), JSON.toJSONString(cliRequest.commandLine));
        } catch (ParseException e) {
    ...
    PlexusContainer container(CliRequest cliRequest) throws Exception {
        ...
        logger.info("创建 ClassRealm:{}", JSON.toJSONString(coreRealm));
        List<File> extClassPath = parseExtClasspath(cliRequest);
        logger.info("获取 extClassPath：{}", extClassPath);
        CoreExtensionEntry coreEntry = CoreExtensionEntry.discoverFrom(coreRealm);
        logger.info("解析META-INF/maven/extension.xml：{}", JSON.toJSONString(coreEntry));
        List<CoreExtensionEntry> extensions =
            loadCoreExtensions(cliRequest, coreRealm, coreEntry.getExportedArtifacts());
        logger.info("loadCoreExtensions：{}", JSON.toJSONString(coreEntry));
     ...
     * have the same artifactId, effective failed module might be later in build reactor. This means that developer will
     * either have to type groupId or wait for build execution of all modules which were fine, but they are still before
     * one which reported errors.
     * <p>
     * Then the returned value is {@code groupId:artifactId} when there is a name clash and {@code :artifactId} if there
     * is no conflict.
     *
     * @param mavenProjects
     *            Maven projects which are part of build execution.
     * @param failedProject
     *            Project which has failed.
     * @return Value for -rf flag to resume build exactly from place where it failed ({@code :artifactId} in general and
     *         {@code groupId:artifactId} when there is a name clash).
     */
    ...
    private MavenExecutionRequest populateRequest(CliRequest cliRequest, MavenExecutionRequest request) {
        ...
        logger.info("baseDirectory:{}", baseDirectory.getPath());
        request.setBaseDirectory(baseDirectory).setGoals(goals).setSystemProperties(cliRequest.systemProperties)
            .setUserProperties(cliRequest.userProperties).setReactorFailureBehavior(reactorFailureBehaviour) // default:
                                                                                                             // fail
                                                                                                             // fast
        ...
        logger.info("localRepoProperty:{}", localRepoProperty);
    ...
    static void populateProperties(CommandLine commandLine, Properties systemProperties, Properties userProperties) {
        logger.info("start-- systemProperties:{},userProperties:{}", JSON.toJSONString(systemProperties),
        ...
        if (commandLine.hasOption(CLIManager.SET_SYSTEM_PROPERTY)) {
            ...
            logger.info("用户输入的参数，例如-Dval=123：{}", defStrs);
            if (defStrs != null) {
        ...
        logger.info("end-- systemProperties:{},userProperties:{}", JSON.toJSONString(systemProperties),
            JSON.toJSONString(userProperties));
    }
